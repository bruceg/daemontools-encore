...
int main(argc,argv) int argc; const char *const *argv;
{
  ...
  if ((wr_null = open_write("/dev/null")) == -1)
    strerr_die2sys(112,FATAL,"unable to open null writer: ");
  ...
  if (close(pi[0]))
    strerr_die2sys(112,FATAL,"unable to close pipe reader: ");
  if (fd_move(0,rd_null))
    strerr_die2sys(112,FATAL,"unable to move null to stdin: ");
  if (fd_move(1,pi[1]))
    strerr_die2sys(112,FATAL,"unable to move pipe to stdout: ");
  if (close(wr_null))
    strerr_die2sys(112,FATAL,"unable to close null writer: ");
  ...
  while (1) {
    ...
    if (ioctl(1,FIONREAD,&remain) == -1)
      strerr_die2sys(112,FATAL,"unable to check pipe: ");
