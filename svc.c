...
int main(int argc,const char *const *argv)
{
  ...
  while ((opt = getopt(argc,argv,"+udopchaitkxq12")) != opteof)
    if (opt == '?')
      strerr_die1x(100,"svc options: + kill process group, u up, d down, o once, x exit, p pause, c continue, h hup, a alarm, i interrupt, t term, k kill, q quit, 1 SIGUSR1, 2 SIGUSR2");
    else
